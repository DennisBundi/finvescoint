        ...
        <div className="flex flex-col border-t border-border">
          {PILLARS.map(({ num, title, subtitle, href }, i) => (
            <motion.div key={num}
              ...
              <Link href={href}
                className="group flex items-center justify-between py-6 md:py-8 border-b border-border hover:border-gold/40 transition-colors duration-300">
                <div className="flex items-center gap-6 md:gap-16">
                  <span className="font-mono text-xs text-gold/40 tracking-widest w-8 flex-shrink-0">{num}</span>
                  <div>
                    <h3 className="font-serif text-2xl sm:text-3xl md:text-5xl text-offwhite italic group-hover:text-gold transition-colors duration-300">
                      {title}
                    </h3>
